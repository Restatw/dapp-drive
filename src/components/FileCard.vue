...
<template>
  <div
    class="relative group flex flex-col rounded-xl border border-gray-100 bg-white hover:shadow-md hover:border-gray-200 cursor-pointer transition-all select-none overflow-visible"
    @dblclick="$emit('open')"
  >
    <!-- Thumbnail / icon area -->
    ...
    <div class="absolute top-1.5 right-1.5 opacity-0 group-hover:opacity-100 transition z-10">
      ...
      <div
        v-if="showMenu"
        class="absolute right-0 top-7 w-40 bg-white rounded-xl shadow-xl border border-gray-100 py-1 z-50 text-left"
        @click.stop
      >
        <button v-if="!isFolder" class="context-item" @click="showMenu = false; $emit('preview')">
          <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
          Preview
        </button>
        <button v-if="!isFolder" class="context-item" @click="showMenu = false; $emit('download')">
          <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
          Download
        </button>
        <button v-if="!isFolder" class="context-item" @click="showMenu = false; $emit('share')">
          <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"/></svg>
          Copy link
        </button>
        <hr class="my-1 border-gray-100" />
        <button class="context-item" @click="showMenu = false; $emit('rename')">
          <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
          Rename
        </button>
        <button class="context-item text-red-600 hover:bg-red-50" @click="showMenu = false; $emit('delete')">
